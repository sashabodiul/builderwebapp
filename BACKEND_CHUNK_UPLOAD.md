...
## Инструкция для бэкенда: ускорение эндпоинтов завершения

Ниже — рекомендации, которые ускоряют `work-shifts/end` и `work/end`, **не меняя бизнес‑логику**.

### 1) `/api/v1/work-shifts/end` (end_work_shift)

**Что медленно:**
- `WorkProcess.filter(... end_time__isnull=False)` и цикл подсчёта времени.

**Ускорение без изменения логики:**

1. **Агрегация на уровне БД**
   - Вместо перебора в Python используйте агрегацию:
     - `SUM(end_time - start_time)` для процессов в смене.
   - Это снижает время на большие смены/объекты.

2. **Индексы**
   - Добавить/проверить индексы:
     - `WorkShift(worker_id, end_time, is_voided)`
     - `WorkProcess(shift_id, end_time, is_voided)`
   - Ускоряет поиск активной смены и выборку процессов.

3. **Минимизация запросов**
   - `Worker.get_or_none` нужен только для `rate`, можно загрузить только `rate`.
   - Использовать `.only("rate")` или аналог ORM.

### 2) `/api/v1/work/end` (end_work)

**Что медленно:**
- валидация/перекладывание файлов,
- загрузка в CRM,
- `prefetch_related` большого графа сущностей.

**Ускорение без изменения результата:**

1. **Убрать тяжёлый `prefetch_related` из ответа**
   - Сейчас ответ строится после `prefetch_related(...)`.
   - Это дорого и редко нужно в момент завершения.
   - Решение:
     - возвращать `work_process` без префетча,
     - при необходимости фронт делает отдельный GET `/work/{id}`.

2. **Делегировать загрузку медиа в фон**
   - Сама логика сохраняется, но можно:
     - записать “pending media upload”,
     - запустить фоновой таск (Celery/RQ/BackgroundTasks),
     - по готовности обновить `*_url`.
   - Ответ `/work/end` возвращается быстрее (без ожидания CRM).

3. **Ускорить работу с `chunked upload`**
   - После получения `file_id` из chunk‑хранилища:
     - передавать поток напрямую в CRM,
     - не писать временный файл на диск, если это возможно.
   - Если без временного файла нельзя (валидация длительности видео):
     - сохранять временный файл 1 раз,
     - использовать его для валидации и загрузки.

4. **Оптимизация проверки геозоны**
   - Сейчас по каждому авто делается запрос `vehicle_service`.
   - Возможные оптимизации:
     - кэш последней геозоны на 10–30 сек,
     - сокращение количества запросов при наличии валидного `is_in_geofence`.

5. **Индексы**
   - Проверить индексы:
     - `WorkProcess(worker_id, end_time, is_voided)`
     - `WorkProcess(shift_id, end_time, is_voided)`
     - `FacilityBudget(facility_id)`

### 3) Общие правила (без изменения логики)

1. **Не делать лишних сериализаций**
   - Не формировать большие вложенные структуры в ответе.
2. **Логирование — только критичное**
   - Уменьшить логирование внутри горячих циклов.
3. **Профилирование**
   - Добавить тайм‑метки на ключевых этапах:
     - геозона,
     - обработка видео,
     - загрузка медиа,
     - расчёт бюджета,
...
Самый быстрый путь без поломок:
- убрать `prefetch_related` из ответа `/work/end`;
- вынести загрузку медиа в фоновые задачи;
- агрегировать время смены в БД;
- добавить индексы.
